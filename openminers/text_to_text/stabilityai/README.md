...
# Example Usage
```
python3 -m pip install -r openminers/text_to_text/miner/stabilityai/requirements.txt
python3 openminers/text_to_text/miner/stabilityai/miner.py --stabilityai.api_key <Your HuggingFace API Key>
```

# Full Usage
```
usage: miner.py [-h] [--stabilityai.api_key STABILITYAI.API_KEY] [--stabilityai.model_size {3,7}]
                 ...
                 [--netuid NETUID] [--miner.name NEURON.NAME] [--miner.blocks_per_epoch NEURON.BLOCKS_PER_EPOCH]
                 [--miner.no_set_weights] [--miner.max_batch_size NEURON.MAX_BATCH_SIZE]
                 [--miner.max_sequence_len NEURON.MAX_SEQUENCE_LEN]
                 [--miner.blacklist.hotkeys [NEURON.BLACKLIST.HOTKEYS [NEURON.BLACKLIST.HOTKEYS ...]]]
                 [--miner.blacklist.allow_non_registered]
                 [--miner.blacklist.default_stake NEURON.BLACKLIST.DEFAULT_STAKE]
                 [--miner.default_priority NEURON.DEFAULT_PRIORITY] [--wallet.name WALLET.NAME]
